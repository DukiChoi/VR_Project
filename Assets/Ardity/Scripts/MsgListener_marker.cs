...
public class MsgListener_marker : MonoBehaviour
{
    ...
    void OnMessageArrived(string msg)
    {
        ...
        if (vec3[0] == "2" && vec3[1] != "" && vec3[2] != "" && vec3[3] != "" && vec3[4] != "") //Check if all values are recieved
        {

            ori_x = float.Parse(vec3[3]);
            //동서남북 방향에 따라서 값 달라짐
            ori_y = float.Parse(vec3[1]) + SettingController.direction;
            ori_z = float.Parse(vec3[2]);
